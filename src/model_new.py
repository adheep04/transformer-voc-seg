...
class SwinSeg(nn.Module):
    def __init__(self, n_class):        
        ...
        self.smooth16 = nn.Conv2d(96, 96, 3, 1, padding=1)
        ...
        # 256 -> class size
        self.score4 = nn.Conv2d(96, n_class, 1)
        self.score8 = nn.Conv2d(192, n_class, 1)
        self.score16 = nn.Conv2d(384, n_class, 1)
        self.score32 = nn.Conv2d(768, n_class, 1)
        ...
        nn.init.kaiming_uniform_(self.score4.weight)
    ...
    def forward(self, x, weights=[1.3, 0.5, 0.3, 0.3]):
        ...
        fm768_raw = self.permute(self.norm768(x))
        
        ''' feature pyramid network '''
        
        # project to 384 dimensions and normalize
        fm96 = self.relu(self.norm384(self.fm96_proj(fm96_raw)))
        fm192 = self.relu(self.norm384(self.fm192_proj(fm192_raw)))
        fm384 = self.relu(self.norm384(self.fm384_proj(fm384_raw)))
        fm768 = self.relu(self.norm384(self.fm768_proj(fm768_raw)))
        
        # feature pyramid
        fuse16 = self.smooth16(fm384 + F.interpolate(fm768, size=fm384.shape[-2:], mode='bilinear', align_corners=False))
        fuse16 = self.relu(self.norm384(self.gate16(torch.cat([fuse16, fm384_raw], dim=1))))
        fuse8 = self.smooth8(fm192 + F.interpolate(fuse16, size=fm192.shape[-2:], mode='bilinear', align_corners=False))
        fuse8 = self.relu(self.norm192(self.gate8(torch.cat([fuse8, fm192_raw], dim=1))))
        fuse4 = self.smooth4(fm96 + F.interpolate(fuse8, size=fm96.shape[-2:], mode='bilinear', align_corners=False))
        fuse4 = self.relu(self.norm96(self.gate4(torch.cat([fuse4, fm96_raw], dim=1))))
        
        # score: dim -> num_classes
        score32 = F.interpolate(self.score32(fm768_raw), size=img_res, mode='bilinear', align_corners=False)
